...
export function AppShell({
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-canvas">
      <header className="flex h-14 shrink-0 items-center border-b border-edge px-6">
      ...
      <div className="flex min-h-0 flex-1">
        <aside
          className="flex w-56 shrink-0 flex-col border-r border-edge bg-surface py-3"
          ...
          {WIZARD_STEPS.map((s, i) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 px-4 py-2.5 text-left text-sm font-medium transition-colors duration-150 ${active
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          <div className="min-h-0 flex-1 overflow-y-auto">
            <div className="mx-auto w-full max-w-[min(100%,72rem)] px-6 py-6 md:px-8">{children}</div>
          </div>

          <footer className="shrink-0 border-t border-edge bg-canvas">
            <div className="mx-auto flex w-full max-w-[min(100%,72rem)] items-center justify-between gap-4 px-6 py-4 md:px-8">
            ...
            {showNext ? (
              ...
              <p className="max-w-lg text-right text-sm text-ink-muted">
                Control sends in the workspace above. Back returns to earlier steps.
              </p>
            )}
            </div>
          </footer>
